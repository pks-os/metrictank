...
func (m *MetaRecordIdx) Init() error {
	...
	err = btHelpers.EnsureTableExists(ctx, m.cfg.CreateCf, m.adminClient, "meta_records", map[string]bigtable.GCPolicy{
	...
	err = btHelpers.EnsureTableExists(ctx, m.cfg.CreateCf, m.adminClient, "meta_record_batches", map[string]bigtable.GCPolicy{
